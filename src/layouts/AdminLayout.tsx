...
export const AdminLayout: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-800 flex flex-col md:flex-row font-sans p-4 md:p-6 gap-6 relative select-none">
      ...
      {/* 2. Premium Desktop Collapsible Floating Sidebar (No Layout Shift) */}
        ...
        <div className="absolute left-0 top-0 bottom-0 md:w-20 xl:w-64 md:group-hover:w-64 bg-white/95 backdrop-blur-xl border border-slate-100 rounded-[2rem] p-6 flex flex-col justify-between shadow-[0_20px_50px_rgba(0,84,166,0.04)] group-hover:shadow-[0_20px_60px_rgba(0,84,166,0.12)] transition-all duration-300 ease-in-out flex flex-col justify-between">
          <div className="space-y-8 flex flex-col flex-1 min-h-0">
            ...
            <div className="flex flex-col items-center justify-center text-center gap-3 px-2">
              ...
              <span className="font-bold tracking-tight text-slate-800 text-xs leading-snug text-center max-w-full block truncate xl:block md:hidden md:group-hover:block transition-all duration-300">
            ...
            <nav className="flex flex-col gap-1.5 flex-1 overflow-y-auto scrollbar-none pr-1">
                ...
                <>
                  ...
                  {(user?.rol === 'OFICIAL_DE_CREDITO' || user?.rol === 'CONTADOR' || user?.rol === 'GERENTE_GENERAL' || user?.rol === 'SUPER_ADMIN_SAAS' || user?.rol === 'ADMINISTRADOR') && (
                    ...
                    >
                      ...
                      <span className="xl:inline md:hidden md:group-hover:inline transition-opacity duration-300 whitespace-nowrap truncate">Socios</span>
                    </Link>
                  )}
                  {(user?.rol === 'OFICIAL_DE_CREDITO' || user?.rol === 'CONTADOR' || user?.rol === 'GERENTE_GENERAL' || user?.rol === 'SUPER_ADMIN_SAAS' || user?.rol === 'ADMINISTRADOR') && (
                    <Link
                    ...
                    >
                      ...
                      <span className="xl:inline md:hidden md:group-hover:inline transition-opacity duration-300 whitespace-nowrap truncate">Créditos</span>
                    </Link>
                  )}
                  {user?.rol === 'CAJERO' && (
                    <Link
                    ...
                    >
                      ...
                      <span className="xl:inline md:hidden md:group-hover:inline transition-opacity duration-300 whitespace-nowrap truncate">Ventanilla</span>
                    </Link>
                  )}
                </>
              )}
              ...
              {(user?.rol === 'CONTADOR' || user?.rol === 'GERENTE_GENERAL' || user?.rol === 'SUPER_ADMIN_SAAS' || user?.rol === 'ADMINISTRADOR') && (
                <>
                  <span className="px-3 text-[10px] font-bold text-slate-400 uppercase tracking-widest block mb-2 mt-6 select-none xl:block md:hidden md:group-hover:block whitespace-nowrap transition-all duration-300">
                    Finanzas y Control
                  </span>
                  <Link
                  ...
                  >
                    ...
                    <span className="xl:inline md:hidden md:group-hover:inline transition-opacity duration-300 whitespace-nowrap truncate">Contabilidad</span>
                  </Link>
                </>
              )}
              ...
              {(user?.rol === 'ADMINISTRADOR' || user?.rol === 'GERENTE_GENERAL' || user?.rol === 'SUPER_ADMIN_SAAS') && (
                <>
                  ...
                  >
                    ...
                    <span className="xl:inline md:hidden md:group-hover:inline transition-opacity duration-300 whitespace-nowrap truncate">Parametrización</span>
                  </Link>
                  <Link
                  ...
                  >
                    <Users className="h-5 w-5 shrink-0" />
                    <span className="xl:inline md:hidden md:group-hover:inline transition-opacity duration-300 whitespace-nowrap truncate">Gestión de Equipo</span>
                  </Link>
                </>
              )}
            </nav>
