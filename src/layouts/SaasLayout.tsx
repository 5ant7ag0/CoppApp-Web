...
export const SaasLayout: React.FC = () => {
  ...
  if (!user || user.rol !== 'SUPER_ADMIN_SAAS') {
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-800 flex flex-col md:flex-row font-sans p-4 md:p-6 gap-6 relative select-none">
      ...
      {/* Mobile Sidebar (stacks at top on mobile) */}
      <aside className="w-full bg-white border border-slate-100 rounded-[2rem] p-6 flex flex-col justify-between shadow-sm md:hidden">
        <div className="space-y-6 flex flex-col">
          <div className="flex items-center gap-3 px-2">
            <div className="h-12 w-12 rounded-xl bg-[#0054A6] flex items-center justify-center font-black text-white shadow-md shadow-blue-600/20">
            ...
            {navItems.map((item) => {
              ...
              return (
                <button
                  ...
                  className={
                    isActive
                      ? "w-full flex items-center gap-3 px-4 py-3 rounded-2xl bg-blue-50 text-[#0054A6] font-semibold"
                      : "w-full flex items-center gap-3 px-4 py-3 rounded-2xl text-slate-500 hover:bg-slate-50 hover:text-slate-800 font-medium"
                  }
                >
        ...
        <div className="border-t border-slate-100 pt-4 mt-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="h-9 w-9 rounded-full bg-[#0054A6]/10 border border-[#0054A6]/20 font-black text-xs text-[#0054A6] flex items-center justify-center">
              {initials}
            </div>
      ...
      <aside className="hidden md:block md:w-20 xl:w-64 h-[calc(100vh-3rem)] sticky top-6 shrink-0 group z-50">
        <div className="absolute left-0 top-0 bottom-0 md:w-20 xl:w-64 md:group-hover:w-64 bg-white/95 backdrop-blur-xl border border-slate-100 rounded-[2rem] p-6 flex flex-col justify-between shadow-[0_20px_50px_rgba(0,84,166,0.04)] group-hover:shadow-[0_20px_60px_rgba(0,84,166,0.12)] transition-all duration-300 ease-in-out flex flex-col justify-between">
          <div className="space-y-8 flex flex-col flex-1 min-h-0">
            ...
            <div className="flex flex-col items-center justify-center text-center gap-3 px-2">
              <div className="h-10 w-10 md:h-11 md:w-11 xl:h-16 xl:w-16 rounded-2xl bg-[#0054A6] flex items-center justify-center font-black text-white shadow-lg shadow-blue-600/30 select-none transition-all duration-300 shrink-0">
                <ShieldAlert className="w-5 h-5 xl:w-8 xl:h-8 md:group-hover:w-8 md:group-hover:h-8 transition-all duration-300" />
              </div>
              <div className="text-center xl:block md:hidden md:group-hover:block transition-all duration-300">
                <span className="font-bold tracking-tight text-slate-800 text-sm leading-snug text-center max-w-full block truncate">
                  SaaS Manager
                </span>
                <p className="text-[9px] text-slate-400 font-bold uppercase tracking-wider mt-0.5 block">Super Admin</p>
              </div>
            </div>

            {/* Navigation Links */}
            <nav className="flex flex-col gap-1.5 flex-1 overflow-y-auto scrollbar-none pr-1">
              <span className="px-3 text-[10px] font-bold text-slate-400 uppercase tracking-widest block mb-2 mt-2 select-none xl:block md:hidden md:group-hover:block whitespace-nowrap transition-all duration-300">
                Plataforma SaaS
              </span>
              {navItems.map((item) => {
                const isActive = location.pathname.startsWith(item.path);
                ...
                return (
                  <button
                    ...
                    className={
                      isActive
                        ? "w-full flex items-center gap-3 px-4 py-3 rounded-2xl bg-blue-50 text-[#0054A6] font-semibold transition-all duration-300 shadow-[0_4px_12px_rgba(0,84,166,0.06)] md:justify-center xl:justify-start md:group-hover:justify-start"
                        : "w-full flex items-center gap-3 px-4 py-3 rounded-2xl text-slate-500 hover:bg-slate-50 hover:text-slate-800 font-medium transition-all duration-300 md:justify-center xl:justify-start md:group-hover:justify-start"
                    }
                  >
                    <Icon className="h-5 w-5 shrink-0" />
                    <span className="xl:inline md:hidden md:group-hover:inline transition-opacity duration-300 whitespace-nowrap truncate">{item.name}</span>
                  </button>
                );
              })}
            </nav>
          ...
          <div className="border-t border-slate-100 pt-5 space-y-4">
            <div className="group/profile relative">
              <div className="flex items-center gap-3 px-2 p-2 rounded-2xl hover:bg-slate-50 transition-colors cursor-pointer border border-transparent hover:border-slate-100 md:justify-center xl:justify-start md:group-hover:justify-start">
                ...
                <div className="min-w-0 flex-1 text-left xl:block md:hidden md:group-hover:block transition-all duration-300">
              ...
              {/* Hover Menu */}
              <div className="absolute bottom-0 left-full ml-4 w-[160px] opacity-0 -translate-x-3 invisible group-hover/profile:opacity-100 group-hover/profile:translate-x-0 group-hover/profile:visible transition-all duration-300 ease-out z-50">
