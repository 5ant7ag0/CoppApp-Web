...
export const Perfil: React.FC = () => {
  ...
  // Solicitar OTP para Cambio de Clave (abre modal OTP)
  const handleSolicitarOtp = async () => {
    ...
    try {
      ...
      setIsPasswordFormOpen(false); // No se abren los campos de contraseña aún
      setShowOtpModal(true); // Abrir modal elegante
  ...
  // Validar el OTP ingresado en el modal
  const handleOtpSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setIsPasswordFormOpen(true); // Abrir los campos de nueva clave en la tarjeta de perfil
      showToast('Código verificado con éxito. Ingrese su nueva clave digital.', 'success');
    } catch (err: any) {
      console.error('Error al validar OTP:', err);
      const errMsg = err.response?.data || 'Código OTP incorrecto o expirado.';
      setOtpError(typeof errMsg === 'string' ? errMsg : (errMsg.message || 'Código OTP incorrecto.'));
    } finally {
              ...
              <div className="mt-6 border-t border-slate-50 pt-6 animate-slide-down">
                ...
                <form onSubmit={handlePasswordSubmit} className="space-y-5">
                  <div className="p-4 rounded-2xl bg-[#0054A6]/5 border border-[#0054A6]/15 text-[#0054A6] text-xs leading-relaxed flex items-start gap-3">
                    ...
                    <p className="text-slate-600">El código ha sido validado correctamente. Por favor, ingrese su nueva clave digital.</p>
                  </div>
                  ...
                  {passwordNueva.trim() && passwordConfirmar.trim() && (
